...
/// <summary>
/// IEventPublisher implementation for AdminService.
...
public sealed class RabbitMqEventPublisher : IEventPublisher
{
        ...
        where T : class
    {
        ...
        _logger.LogInformation(
            "[AdminService] Publish requested — EventType: {EventType} → Queue: '{Queue}'",
            eventType, queueName);
        ...
        try
        {
            // ── Processing started ────────────────────────────────────────────
            _logger.LogDebug(
                "[AdminService] Publishing {EventType} — serializing and declaring queue '{Queue}'",
                eventType, queueName);
            ...
            _logger.LogInformation(
                "[AdminService] Published {EventType} → '{Queue}' successfully.",
                eventType, queueName);
        }
        catch (Exception ex)
        {
            // ── Error ─────────────────────────────────────────────────────────
            _logger.LogError(ex,
                "[AdminService] Failed to publish {EventType} → '{Queue}': {ExceptionType}: {Message}",
