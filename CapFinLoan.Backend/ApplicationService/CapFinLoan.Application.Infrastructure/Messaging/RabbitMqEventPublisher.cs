...
/// <summary>
/// IEventPublisher implementation for ApplicationService.
...
public sealed class RabbitMqEventPublisher : IEventPublisher
{
        ...
        where T : class
    {
        ...
        _logger.LogInformation(
            "[ApplicationService] Publish requested — EventType: {EventType} → Queue: '{Queue}'",
            eventType, queueName);
        ...
        try
        {
            // ── Processing started ────────────────────────────────────────────
            _logger.LogDebug(
                "[ApplicationService] Publishing {EventType} — serializing and declaring queue '{Queue}'",
                eventType, queueName);
            ...
            _logger.LogInformation(
                "[ApplicationService] Published {EventType} → '{Queue}' successfully.",
                eventType, queueName);
        }
        catch (Exception ex)
        {
            // ── Error ─────────────────────────────────────────────────────────
            _logger.LogError(ex,
                "[ApplicationService] Failed to publish {EventType} → '{Queue}': {ExceptionType}: {Message}",
